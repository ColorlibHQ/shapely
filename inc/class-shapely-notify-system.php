...
if ( ! class_exists( 'Shapely_Notify_System' ) ) {
	...
	class Shapely_Notify_System extends Epsilon_Notify_System {
		...
		public static function shapely_kaliforms_title() {
			...
			if ( ! $installed ) {
				return esc_html__( 'Install: Kaliforms', 'shapely' );
			}

			$active = self::check_plugin_is_active( 'kali-forms' );
			if ( $installed && ! $active ) {
				return esc_html__( 'Activate: Kaliforms', 'shapely' );
			}
		...
		public static function shapely_kaliforms_description() {
			...
			if ( ! $installed ) {
				return esc_html__( 'Please install Kaliforms. Note that you won\'t be able to use Contact widget without it.', 'shapely' );
			}

			$active = self::check_plugin_is_active( 'kali-forms' );
			if ( $installed && ! $active ) {
				return esc_html__( 'Please activate Kaliforms. Note that you won\'t be able to use Contact widget without it.', 'shapely' );
			}
